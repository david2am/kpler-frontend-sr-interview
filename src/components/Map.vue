...
export default {
  ...
  methods: {
    ...
    addVesselTracksToMap(geojsonData) {
      this.map.on("load", () => {
        this.map && this.map.addSource("vessel-tracks", {
          type: "geojson",
          data: geojsonData,
        });
    ...
    addRedCirclesToMap() {
      this.map && this.map.addSource("country-circles", {
        type: "geojson",
        data: {
          type: "FeatureCollection",
    ...
    updateCirclesOnMap(newCountries) {
      if (this.map && this.map.getLayer("country-circles-layer")) {
        this.map.removeLayer("country-circles-layer");
      }

      if (this.map && this.map.getSource("country-circles")) {
